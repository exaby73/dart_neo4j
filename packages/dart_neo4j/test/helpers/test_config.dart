...
class TestConfig {
  ...
  static String get boltUri =>
      Platform.environment['NEO4J_SINGLE_URI'] ?? 'bolt://localhost:7687';
  static String get neo4jUri =>
      Platform.environment['NEO4J_SINGLE_NEO4J_URI'] ??
      'neo4j://localhost:7687';
  static String get singleUser =>
      Platform.environment['NEO4J_SINGLE_USER'] ?? 'neo4j';
  static String get singlePassword =>
      Platform.environment['NEO4J_SINGLE_PASSWORD'] ?? 'password';

  // Cluster configuration (for neo4j:// routing tests)
  static String get neo4jClusterUri =>
      Platform.environment['NEO4J_CLUSTER_URI'] ?? 'neo4j://localhost:7688';
  static String get clusterUser =>
      Platform.environment['NEO4J_CLUSTER_USER'] ?? 'neo4j';
  static String get clusterPassword =>
      Platform.environment['NEO4J_CLUSTER_PASSWORD'] ?? 'password';

  // SSL configuration (for bolt+s:// and bolt+ssc:// tests)
  static String get boltSslUri =>
      Platform.environment['NEO4J_SSL_URI'] ?? 'bolt+s://localhost:7694';
  static String get boltSelfSignedUri =>
      Platform.environment['NEO4J_SELF_SIGNED_URI'] ??
      'bolt+ssc://localhost:7695';
  static String get sslUser =>
      Platform.environment['NEO4J_SSL_USER'] ?? 'neo4j';
  static String get sslPassword =>
      Platform.environment['NEO4J_SSL_PASSWORD'] ?? 'password';
  ...
  static int get testTimeout =>
      int.tryParse(Platform.environment['TEST_TIMEOUT'] ?? '30000') ?? 30000;
  static int get concurrentSessions =>
      int.tryParse(Platform.environment['TEST_CONCURRENT_SESSIONS'] ?? '50') ??
      50;
  static int get performanceQueries =>
      int.tryParse(
        Platform.environment['TEST_PERFORMANCE_QUERIES'] ?? '1000',
      ) ??
      1000;
  ...
  static Future<void> waitForSingle({
    ...
    throw Exception(
      'Neo4j single instance did not become available within $timeout',
    );
  }

  /// Waits for the Neo4j cluster to be ready
  static Future<void> waitForCluster({
  ...
  static Future<void> waitForNeo4j({
    Duration timeout = const Duration(seconds: 30),
  }) async {
  ...
  static Future<void> waitForNeo4jCluster({
    Duration timeout = const Duration(seconds: 60),
  }) async {
  ...
  static Future<void> waitForSsl({
    ...
    throw Exception(
      'Neo4j SSL instance did not become available within $timeout',
    );
  }

  /// Waits for the self-signed SSL Neo4j instance to be ready
  static Future<void> waitForSelfSigned({
    Duration timeout = const Duration(seconds: 30),
  }) async {
    final deadline = DateTime.now().add(timeout);
    ...
    throw Exception(
      'Neo4j self-signed SSL instance did not become available within $timeout',
    );
  }
}
