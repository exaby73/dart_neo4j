...
class SSLTestHelper {
  ...
  static String _findProjectRoot(String currentPath) {
    ...
    throw Exception('Could not find project root. Ensure dpk.yaml exists in the project root. Current working directory: ${Directory.current.path}');
  }
  ...
  static Future<void> setupSSLContextForTesting() async {
    ...
    if (!await caCertFile.exists()) {
      throw Exception('CA certificate not found at $caCertPath. Run ./scripts/generate-ssl-certs.sh first.');
    }
  ...
  static bool validateTestCertificate(X509Certificate cert) {
    ...
    const expectedIssuer = '/C=US/ST=CA/L=Test/O=Neo4j Test/OU=Testing/CN=Neo4j Test CA';
    ...
    const expectedSubject = '/C=US/ST=CA/L=Test/O=Neo4j Test/OU=Testing/CN=localhost';
